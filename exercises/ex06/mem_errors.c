...
int main()
{
    ...
    read_element(array1, 0); // change -1 to number that falls in bounds
    read_element(array1, 99); // change 100 to number that falls in bound (and not \0)

    // but it does bounds-check dynamic arrays
    read_element(array2, 0); // change -1 to number that falls in bounds
    read_element(array2, 99); // change 100 to number that falls in bound (and not \0)
    free(array2); // array2 is lost outside of main()
    ...
    *never_free = 17;
    free(never_free); // never_free is lost outside of main()
